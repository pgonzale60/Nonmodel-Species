### Repeats annotation ###
# Repeatmodeler
cmd="cd /bio/pmgonza1/mnark/output/quickmerge/canuNplatnus; module load RepeatModeler/1.0.11; BuildDatabase -name genome -engine ncbi qm2.def.fasta; RepeatModeler -srand 2017 -engine ncbi -pa 3 -database genome"
echo "$cmd" | qsub -N RM_canuPiFSC -l nodes=1:ppn=8,walltime=999:00:00,vmem=40gb -q default
genome-families.fa

# Repeatmasker
cmd="cd /bio/pmgonza1/mnark/output/quickmerge/canuNplatnus; source activate py2.7; RepeatMasker -lib genome-families.fa -gff -xsmall -norna -pa 8 qm2.def.fasta"
...
agat_sp_extract_sequences.pl -p --clean_final_stop --clean_internal_stop -g ${outname}.longest_isoform.gtf -f dwil-all-chromosome-r1.05.fasta.gz -o ${outname}.longest_isoform.agat.faa


# Exonerate
exonerate -q ${outname}.longest_isoform.agat.faa -t qm2.def.rmsk.fasta --model protein2genome --querytype protein --targettype dna --showvulgar no --softmaskquery yes --softmasktarget yes --minintron 20 --maxintron 3000 --showalignment no --showtargetgff yes --showcigar no --geneseed 250 --score 250 --verbose 0 --gff3 yes > exonerate.gff3


### De novo trancriptome assembly and merging with exonerated proteins ###
# HISAT2
...
cp /storage/trichoNet/output/PASA/funannotate/*fig /storage/trichoNet/output/PASA/funannotate/run_pasa.sh /storage/Dwillistoni/output/pilot_tissues/StringTie/merged/merged.gtf PASA/exonerate/
...
awk '$3 ~ /^[0-9]+$/ { if($3 > $4){strand = "-"; start=$4+1; end=$3+1} else {strand = "+"; start=$3; end=$4}; printf "%sttRNAscan-SEttRNAt%dt%dt%7.2ft%st.taa=%s;anti=%sn" ,$1,start,end,$9,strand,$5,$6}' tRNAs.tab | gzip > tRNAs.gtf.gz
